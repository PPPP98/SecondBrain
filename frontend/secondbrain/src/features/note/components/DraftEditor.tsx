...
export function DraftEditor({ draftId, isOpen, onClose }: DraftEditorProps) {
  ...
  const { title, content, handleTitleChange, handleContentChange, deleteDraft } = useNoteDraft({
    draftId,
  ...
  return (
    <SidePeekOverlay isOpen={isOpen} onClose={handleClose}>
      <DraftToolbar onBack={handleClose} onDelete={handleDelete} />
