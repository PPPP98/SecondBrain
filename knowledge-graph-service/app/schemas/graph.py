    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                "user_id": "user-123",
                "nodes": [
                    {"id": "note-1", "title": "Neo4j 기초", "created_at": "2025-11-03T12:00:00.000Z"},
                    {"id": "note-2", "title": "그래프 DB", "created_at": "2025-11-03T13:00:00.000Z"}
                ],
                "links": [
                    {"source": "note-1", "target": "note-2", "score": 0.85}
                ],
                "stats": {
                    "total_nodes": 2,
                    "total_links": 1,
...
class NeighborGraphResponse(BaseModel):
    ...
    neighbors: List[NeighborNode] = Field(default_factory=list, description="이웃 노드 리스트")
