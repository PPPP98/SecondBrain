...
class GraphNode(BaseModel):
    ...
    id: int = Field(..., description="노트 ID")
...
class GraphLink(BaseModel):
    ...
    source: int = Field(..., description="출발 노트 ID")
    target: int = Field(..., description="도착 노트 ID")
    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                ...
                "links": [{"source": 1, "target": 2, "score": 0.85}],
...
class NeighborNode(BaseModel):
    ...
    center_id: int = Field(..., description="중심 노트 ID")
    center_title: str = Field(..., description="중심 노트 제목")
    neighbor_id: int = Field(..., description="이웃 노트 ID")
...
class NeighborGraphResponse(BaseModel):
    ...
    center_note_id: int = Field(..., description="중심 노트 ID")
